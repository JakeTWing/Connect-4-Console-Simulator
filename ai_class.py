...
class ai:
    ...
    #ORIGNAL - Alot less effecent, as each time would deep copy
    #min/max move selection  
    ...
    #each time the method is looped it creates a new 'board' object that is then put into the next call of 'simulate'
    ...
    def simulate(self, is_x, prev_board, path_len, n = 0):
        ...
        if n >= path_len:
            return self.eval_pos(prev_board), -1
        
        #main loop, looping through the length of one row
        for i in range(len(prev_board.ret_grid()[0])):
            
            
            new_board = board(prev_board.ret_grid_copy()) #new board object; #different from the previous
            
            valid_move = new_board.place_peice(is_x, i, False) #updating the grid; #attempts to place a peice, if the row is full skip this move
            
            if not(valid_move):
                continue 
            
            win, location, draw = new_board.check_for_win(is_x) #after the move is made, check for a win and a draw. *location is not needed in this method but is still part of the 'win' method for other uses*
            ...
            if win: #if a win is found set score to 5000 - n (reverse for opponent) and stop recursing the path.
                ...
                score  = self.simulate(not is_x, new_board, path_len, n+1)[0] #take only the best score returned from the recursion
